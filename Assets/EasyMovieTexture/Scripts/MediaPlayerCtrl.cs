...
public class MediaPlayerCtrl : MonoBehaviour
{
        ...
        if (m_CurrentState == MEDIAPLAYER_STATE.PLAYING || m_CurrentState == MEDIAPLAYER_STATE.PAUSED)
        {
            if (m_bCheckFBO == false)
            {
                ...
                else
                {
                    if (m_bSupportRockchip)
                    {
                        m_VideoTexture = new Texture2D(Call_GetVideoWidth(), Call_GetVideoHeight(),
				...
				if (m_bSupportRockchip) {
					m_VideoTexture =
 new Texture2D (Call_GetVideoWidth (), Call_GetVideoHeight (), TextureFormat.RGB565, false);
					
				} else {
					m_VideoTexture =
 new Texture2D (Call_GetVideoWidth (), Call_GetVideoHeight (), TextureFormat.RGBA32, false);
				}
				m_VideoTexture.filterMode = FilterMode.Bilinear;
				m_VideoTexture.wrapMode = TextureWrapMode.Clamp;
	...
	bFirstIOS = true;
        ...
        if (m_bPC_FastMode == true)
        {
            ...
            m_VideoTextureU = new Texture2D(Call_GetVideoWidth() / 2, Call_GetVideoHeight() / 2, TextureFormat.Alpha8,
                false);
            m_VideoTextureV = new Texture2D(Call_GetVideoWidth() / 2, Call_GetVideoHeight() / 2, TextureFormat.Alpha8,
                false);
        }
        else
        {
                ...
                if (pPacket->stream_index == iStreamIndex)
                {
                    ...
                    if (gotPicture == 1)
                    {
                        ...
                        if (pCodecContext->width != m_iWidth)
                        {
                            ...
                            pConvertContext = ffmpeg.sws_getContext(pCodecContext->width, pCodecContext->height,
                                sourcePixFmt,
                            ...
                            //pConvertedFrame = ffmpeg.avcodec_alloc_frame();
                            pConvertedFrame = ffmpeg.av_frame_alloc();
                            var convertedFrameBufferSize = ffmpeg.avpicture_get_size(convertToPixFmt,
                                pCodecContext->width, pCodecContext->height);
                            pConvertedFrameBuffer = (sbyte*) ffmpeg.av_malloc((ulong) convertedFrameBufferSize);
                            AVPicture* tempPicture = (AVPicture*) pConvertedFrame;
                            ffmpeg.avpicture_fill(tempPicture, pConvertedFrameBuffer, convertToPixFmt,
                                pCodecContext->width, pCodecContext->height);
                        }
                        else
                        {
                            if (m_bPC_FastMode == true)
                            {
                                ...
                                if (iLineSize != m_iWidth)
                                {
                                    ...
                                    for (int i = 0; i < m_iHeight / 2; i++)
                                    {
                                        ...
                                        imageBufferPtr = new IntPtr(convertedFrameAddress);
                                        Marshal.Copy(imageBufferPtr, buffer, m_iWidth * m_iHeight + m_iWidth / 2 * i,
                                            m_iWidth / 2);

                                        convertedFrameAddress = pDecodedFrame->data2 + iLineSize2 * i;
                                        imageBufferPtr = new IntPtr(convertedFrameAddress);
                                        Marshal.Copy(imageBufferPtr, buffer,
                                            m_iWidth * m_iHeight + m_iWidth / 2 * m_iHeight / 2 + m_iWidth / 2 * i,
                                            m_iWidth / 2);
                                    }
                                }
                                else
                                {
                                    ...
                                    imageBufferPtr = new IntPtr(convertedFrameAddress);
                                    Marshal.Copy(imageBufferPtr, buffer, m_iWidth * m_iHeight,
                                        m_iWidth / 2 * m_iHeight / 2);

                                    convertedFrameAddress = pDecodedFrame->data2;
                                    imageBufferPtr = new IntPtr(convertedFrameAddress);
                                    Marshal.Copy(imageBufferPtr, buffer,
                                        m_iWidth * m_iHeight + m_iWidth / 2 * m_iHeight / 2,
                                        m_iWidth / 2 * m_iHeight / 2);
                                }
    ...
    private void Call_UpdateVideoTexture()
    {
        ...
        if (bEnd == false)
        {
            ...
            if (threadVideo2 == null && m_CurrentState != MEDIAPLAYER_STATE.END &&
                m_CurrentState != MEDIAPLAYER_STATE.NOT_READY && m_CurrentState != MEDIAPLAYER_STATE.ERROR)
            {
                threadVideo2 = new Thread(ThreadUpdate2);
                threadVideo2.IsBackground = true;
        ...
        if (fLastFrameTime > fCurrentSeekTime - 0.2f)
        {
            for (int i = 0; i < listAudio.Count; i++)
            {
                ...
                {
                    if (audioSource == null && (int) ((float) pAudioCodecContext->sample_rate *
                                                      ((float) listAudioPtsTime[i] +
                                                       ((float) Call_GetDuration() / 1000.0f))) > 0)
                    {
...
#if UNITY_5
						audioClip =
 AudioClip.Create("videoAudio",(int)((float)pAudioCodecContext->sample_rate * 300.0f),pAudioCodecContext->channels,pAudioCodecContext->sample_rate,false);


#else
                        audioClip = AudioClip.Create("videoAudio",
                            (int) ((float) pAudioCodecContext->sample_rate * 300.0f), pAudioCodecContext->channels,
                        ...
                        {
                            {
                                ...
                                else*/
                                {
                                    ...
                                    audioClip.SetData(listAudio[i],
                                        (int) (listAudioPts[i] % (pAudioCodecContext->sample_rate * 300.0f)));
	...
	audioSource.time = fLastFrameTime;
                ...
                //if (m_bSetTextureFirst == false)
                {
                    ...
                    if (m_TargetMaterial != null)
                    {
                        for (int i = 0; i < m_TargetMaterial.Length; i++)
                        {
                            ...
                            if (m_TargetMaterial[i].GetComponent<MeshRenderer>() != null)
                            {
                                if (m_TargetMaterial[i].GetComponent<MeshRenderer>().material.mainTexture !=
                                    m_VideoTexture)
                                {
                                    m_TargetMaterial[i].GetComponent<MeshRenderer>().material.mainTexture =
                                        m_VideoTexture;
                                }
                            }
                            ...
                            if (m_bPC_FastMode == true)
                            {
                                if (m_TargetMaterial[i].GetComponent<MeshRenderer>() != null)
                                {
                                    material.mainTextureScale = m_TargetMaterial[i].GetComponent<MeshRenderer>()
                                        .material.mainTextureScale;
                                    material.mainTextureOffset = m_TargetMaterial[i].GetComponent<MeshRenderer>()
                                    ...
                                    if (m_TargetMaterial[i].GetComponent<MeshRenderer>().material.GetTexture("_Y") !=
                                        m_VideoTextureY)
                                    {
                                        m_TargetMaterial[i].GetComponent<MeshRenderer>().material
                                            .SetTexture("_Y", m_VideoTextureY);
                                    }
                            ...
                            if (m_bPC_FastMode == true)
                            {
                                if (m_TargetMaterial[i].GetComponent<RawImage>() != null)
                                {
                                    ...
                                    if (m_TargetMaterial[i].GetComponent<RawImage>().material.GetTexture("_Y") !=
                                        m_VideoTextureY)
                                    {
                                        m_TargetMaterial[i].GetComponent<RawImage>().material
                                            .SetTexture("_Y", m_VideoTextureY);
                                    }
                ...
                if (bVideoFirstFrameReady == true)
                {
                    ...
                    if (listAudio.Count > 0)
                    {
                        if (
                            audioSource ==
                            null /*&& (int)((float)pAudioCodecContext->sample_rate * ((float)listAudioPtsTime[i] + ((float)Call_GetDuration() / 1000.0f))) > 0*/
                        )
                        {
                            audioSource = gameObject.AddComponent<AudioSource>();
                        }
 ...
 AudioClip.Create("videoAudio", (int)((float)pAudioCodecContext->sample_rate * 300.0f), pAudioCodecContext->channels, pAudioCodecContext->sample_rate, false);
...
#else
                                audioClip = AudioClip.Create("videoAudio",
                                    (int) ((float) pAudioCodecContext->sample_rate * 300.0f),
                                    pAudioCodecContext->channels, pAudioCodecContext->sample_rate, false, false);
    ...
    private void Call_Play(int iSeek)
    {
        if (m_CurrentState == MEDIAPLAYER_STATE.READY || m_CurrentState == MEDIAPLAYER_STATE.END ||
            m_CurrentState == MEDIAPLAYER_STATE.PAUSED)
        {
            SeekTo(iSeek);
            if (audioSource != null)
    ...
    private int Call_GetCurrentSeekPercent()
    {
        if (pStream != null)
        {
            return (int) (((double) fCurrentSeekTime +
                           ((double) m_queuePacketVideo.Count / av_q2d(pStream->avg_frame_rate))) /
                (Call_GetDuration() / 1000.0) * 100);
        }
